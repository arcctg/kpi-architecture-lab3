...
public class ListCardsUseCase {
    ...
    public DomainPage<CardResult> execute(Long deckId, String ownerEmail,
        ...
        DomainPage<Card> cards;
        if (search != null && !search.isBlank()) {
            cards = cardRepository.findByDeckIdAndTermContaining(deckId, search, page, size);
        } else {
            cards = cardRepository.findByDeckId(deckId, page, size);
        }

        return new DomainPage<>(
                cards.content().stream().map(CardResultMapper::toResult).toList(),
                cards.page(),
                cards.size(),
                cards.totalElements(),
                cards.totalPages()
